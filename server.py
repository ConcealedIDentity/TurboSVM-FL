...
def federated_learning(args, train_clients, test_clients, global_model):
    ...
    for current_global_epoch in tqdm.tqdm(range(args.global_epoch)):
        ...
        for client, client_model in zip(update_clients, client_models):
            previous_features = client.local_train(client_model, global_model, previous_features, current_global_epoch, args.global_epoch)
...
def server_eval(clients, wandb_log, metric_prefix):
    labels = []
    preds  = []
    for c in clients:
        l, p = c.local_eval()
...
# ours
def Ours(global_model, client_models, client_weights, global_optim, logits_optim, current_global_epoch, num_global_epoch, class_C, base_agg, agg_svc, spreadout):
